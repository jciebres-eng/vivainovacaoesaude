...
import { apoiosDePreparacao, novoId, type Atividade, type Preparacao } from "@/lib/viva-percurso";
...
const etapas = ["O que vou fazer", "O que pode ajudar", "Como estou agora", "Meu plano"];
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 space-y-5">
          ...
          <>
            ...
            <fieldset>
              ...
              <p className="mt-1 viva-legenda text-text-secondary">
                Uma alternativa não é uma versão menor. É outra forma de participar.
              </p>
              <div className="mt-3 space-y-2">
                {atividade.alternativas.map((alt) => (
                  <CaixaDeSelecao
                    ...
                    onChange={(v) => mudar({ alternativaEscolhida: v ? alt : undefined })}
                  />
                ))}
              </div>
            </fieldset>
        ...
        {etapa === 1 ? (
          <fieldset>
            <legend className="viva-rotulo text-text-primary">O que pode ajudar (opcional)</legend>
            <p className="mt-1 viva-legenda text-text-secondary">
            ...
            <div className="mt-3 space-y-3">
              {apoiosDePreparacao.map((a) => (
                <CaixaDeSelecao
                  ...
                  onChange={(v) => {
                    setApoios((prev) => (v ? [...prev, a] : prev.filter((x) => x !== a)));
                    setSalvo(false);
                  }}
                />
              ))}
            ...
            {estrategiasDisponiveis.length ? (
              <div className="mt-6">
                <p className="viva-rotulo text-text-primary">Minhas estratégias já registradas</p>
                <div className="mt-3 space-y-3">
                      ...
                      onChange={(v) =>
                        mudar({
                          estrategiaIds: v
                            ...
                            : preparacao.estrategiaIds.filter((id) => id !== e.id),
                        })
                      }
                    />
                  ))}
        ...
        {etapa === 2 ? (
          <>
            <p className="viva-legenda text-text-secondary">
              Este registro é opcional e serve só para você. Não é avaliação clínica e não é
            ...
            <ControleDeslizante
              ...
              onChange={(v) => mudar({ comoEstou: { ...preparacao.comoEstou, energia: v } })}
            />
            <ControleDeslizante
              rotulo="Desconforto agora"
              valor={preparacao.comoEstou.desconforto ?? 3}
              formatarValor={(v) => `${v} de 10`}
              onChange={(v) => mudar({ comoEstou: { ...preparacao.comoEstou, desconforto: v } })}
            />
            <ControleDeslizante
        ...
        {etapa === 3 ? (
          <>
            <dl className="space-y-3 rounded-2xl bg-surface-muted p-4 viva-apoio">
              ...
              />
              <Linha rotulo="Apoio disponível" valor={preparacao.apoio || "Não registrado"} />
              <Linha
                rotulo="Posso pausar"
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        {etapa < etapas.length - 1 ? (
          ...
          <Botao variante="principal" onClick={() => salvar(onComecarAtividade)}>
            Começar atividade
          </Botao>
        )}
        {etapa > 0 ? (
